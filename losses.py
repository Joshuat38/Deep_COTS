...
class YOLOXLoss(nn.Module):
    def __init__(self, label_name, reid_dim=0, id_nums=None, 
                 strides=[8, 16, 32], in_channels=[256, 512, 1024], 
                 use_l1_epoch=None, verbose=1):
        super().__init__()
        ...
        self.use_l1_epoch = use_l1_epoch
    ...
    def forward(self, preds, targets, total_loss, losses, imgs=None):
        ...
        losses = {"Confidence_Loss": conf_loss.item(), "Class_Loss": cls_loss.item(), "IOU_Loss": iou_loss.item()}
        # else:
        #     losses = {"Confidence_Loss": conf_loss.item(), "IOU_Loss": iou_loss.item()}
        if self.use_l1:
            losses.update({"l1_loss": l1_loss.item()})
        if self.reid_dim > 0:
            losses.update({"reid_loss": reid_loss.item()})
    ...
    def update_epoch(self, current_epoch):
        ...
        if self.verbose == 1:
            print(f'\nYOLOX Loss registered current epoch as: {self.current_epoch+1}.')
            
        if self.current_epoch >= self.use_l1_epoch:
            self.use_l1 = True
            if self.verbose == 1:
                print('L1 loss is now enabled!')
    ...
    def get_losses(self, imgs, x_shifts, y_shifts, expanded_strides, targets, outputs, origin_preds, dtype):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_classes = targets[batch_idx, :num_gt, 4] # Corrected to the data configurations.
                gt_bboxes_per_image = targets[batch_idx, :num_gt, 0:4] # Corrected to the data configurations.
                if self.reid_dim > 0:
